...
void solve(){
   ...
   while(s>0)
   {
      ...
      int rema=a%10;
        ...
        if(rems>=rema){res=to_string(rems-rema)+res;}
        else if(rema>rems)
        {
           ...
           if(rems>rema&&rems>=10&&rems<=19){ res=to_string(rems-rema)+res;}
           else{cout<<"-1"<<endl;return;}

        }
         s/=10;
        a/=10;
        // if(s==0&&a>=1){res="-1";break;}

   }
  if(a){cout<<"-1"<<endl;return;}
   string t="";
   int flag=0;
   forf(int,i,res.length()){
